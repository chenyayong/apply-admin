    ...
    <Table v-loading="loading" size="small" :data="tabelData" :current-page.sync="formData.page" :page-size.sync="formData.limit" :total="total" @change="paginationChange">
      ...
      <el-table-column label="操作" min-width="180px" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button size="mini" @click="handleSubmit(scope)" v-if="scope.row.status === 2">提交微信</el-button>
...
import { getWeChatList, toReg } from '@/api/applys'
...
export default class extends Vue {
  ...
  async getWeChatList() {
    this.loading = true
    try {
      const res = await getWeChatList(this.formData)
      const data = res.data || {}
  ...
  async handleSubmit(scope: { row: IApply }) {
    this.loading = true
    try {
      const res = (await toReg(scope.row.apply_id)) as any
      if (res.code === 20000) {
        this.getWeChatList()
      }
    } catch (error) {
